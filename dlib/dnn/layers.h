...
// ----------------------------------------------------------------------------------------
	...
	public:
		...
		{
			data_output.copy_size(sub.get_output());
			tt::leakyrelu(data_output, sub.get_output(), params);
		}
		...
		{
			tt::leakyrelu_gradient(sub.get_gradient_input(), sub.get_output(),
				gradient_input, params, params_grad);
		}
		...
		friend void serialize(const leakyrelu_& item, std::ostream& out)
		{
			serialize("leakyrelu_", out);
			serialize(item.params, out);
			serialize(item.initial_param_value, out);
		}

		friend void deserialize(leakyrelu_& item, std::istream& in)
		{
			std::string version;
			deserialize(version, in);
			if (version != "leakyrelu_")
				throw serialization_error("Unexpected version '" + version + "' found while deserializing dlib::prelu_.");
			deserialize(item.params, in);
			deserialize(item.initial_param_value, in);
		}

		friend std::ostream& operator<<(std::ostream& out, const leakyrelu_& item)
		{
			out << "leakyrelu\t ("
				<< "initial_param_value=" << item.initial_param_value
				<< ")";
		...
		friend void to_xml(const leakyrelu_& item, std::ostream& out)
		{
			out << "<leakyrelu initial_param_value='" << item.initial_param_value << "'>\n";
			out << mat(item.params);
			out << "</leakyrelu>\n";
		}
	...
	template <typename SUBNET>
	using leakyrelu = add_layer<leakyrelu_, SUBNET>;
// ----------------------------------------------------------------------------------------
    class sig_
    {
    public:
