...
{ 
    ...
    // ----------------------------------------------------------------------------------------
		...
		{
			launch_kernel(_cuda_leakyrelu, max_jobs(dest.size()),
				src.device(), dest.device(), src.size(), param.device());
		}
    // ----------------------------------------------------------------------------------------
		...
		{
			params_grad = 0;
			launch_kernel(_cuda_leakyrelu_gradient, max_jobs(grad.size()),
				grad.device(), src.device(), gradient_input.device(), grad.size(),
				param.device(), params_grad.device());
		}
    ...
    // ----------------------------------------------------------------------------------------
        ...
        __global__ void _cuda_compute_loss_multiclass_log_per_pixel(float* loss_out, float* g, const uint16_t* truth, size_t n, size_t plane_size, size_t sample_size, size_t nk, const uint16_t label_to_ignore,const float scale)
        {
			float loss = 0;
			for (auto i : grid_stride_range(0, n))
			{
		...
		__device__ double cuda_log1pexp(double x)
		{
			using std::exp;
		...
		__global__ void _cuda_compute_loss_binary_log(float* loss_out, float* g, const float* truth, size_t sample_size, const float scale,const float* out_data)
		{
			...
			for (long i = 0; i < sample_size; ++i)
			{
				...
				if (y > 0)
				{
					temp = cuda_log1pexp(-out_data[i]);
					loss += y * scale * temp;
					g[i] = y * scale * (g[i] - 1);
				}
				else
				{
					temp = -(-out_data[i] - cuda_log1pexp(-out_data[i]));
					loss += -y * scale * temp;
					g[i] = -y * scale * g[i];
				}
			}
			...
			)
		{
			...
			launch_kernel(_cuda_compute_loss_binary_log, max_jobs(gradient.size()),
